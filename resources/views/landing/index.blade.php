<x-app-layout>
            ...
            <div style="box-shadow: 0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgb(0 0 0 / 0.05);"
                ...
                <div class="py-4 px-2">
                    <div class="flex flex-col space-y-1.5 pb-6">
                        <h2 class="font-semibold text-lg tracking-tight">Chat CIMB</h2>
    ...
    <x-slot name="scripts">
        ...
        <script>
            ...
            function generateChatHTML(type, text) {
                var html = '';
                if (type === 'user') {
                    html = `<div class="flex gap-3 my-4 text-gray-600 text-sm flex-1"><span
                            ...
                            <p class="leading-relaxed"><span class="block font-bold text-gray-700">You </span>${text}
                            </p>
                        </div>`;
                } else if(type === 'ai') {
                    html = `<div class="flex gap-3 my-4 text-gray-600 text-sm flex-1"><span
                            ...
                            <p class="leading-relaxed"><span class="block font-bold text-gray-700">AI </span>${text}
                            </p>
                        </div>`;
                }
            ...
            document.getElementById('form-ai').addEventListener('submit', (e) => {
                ...
                if (input == '') return;
                const aiContainer = document.getElementById('chat-ai-container');
                let html = '';
                ...
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition((position) => {
                        ...
                        axios.post('{{ route('api.askbot') }}', {
                            ...
                            if (resp.data.data.aiGenerated) {
                                aiContainer.insertAdjacentHTML('beforeend', generateChatHTML('ai', resp.data.data.aiGenerated));
                            }

                            if (resp.data.data.nearestBranches.length > 0) {
                                resetMap();
                                addMarkersToMap(resp.data.data.nearestBranches);
                            }
                        }).catch((e) => {
                            displayLoading(false);
                            aiContainer.insertAdjacentHTML('beforeend', generateChatHTML('ai', 'Input Error'));
                            alert(e.response)
                            console.error(e)
                        }).finally((e) => {
                            displayLoading(false);
                    ...
                    aiContainer.insertAdjacentHTML('beforeend', generateChatHTML('ai', 'Permission locaiton denied'));
                }
            });
