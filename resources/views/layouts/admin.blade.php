            ...
            <nav :class="{ 'block': open, 'hidden': !open }"
                ...
                <div @click.away="open = false" class="relative" x-data="{ open: false }">
                    <button @click="open = !open"
                        class="flex opacity-50 flex-row items-center w-full px-4 py-2  mt-2 text-sm font-semibold text-left bg-transparent rounded-lg md:block hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:opacity-100 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
                        <span>More</span>
                        <svg fill="currentColor" viewBox="0 0 20 20" :class="{ 'rotate-180': open, 'rotate-0': !open }"
                            class="inline w-4 h-4 mt-1 ml-1 transition-transform duration-500 transform md:-mt-1">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-500"
                        ...
                        <div class="px-2 py-2 bg-white rounded-md shadow">
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <a href="/"
                                    class="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline">Homepage</a>
                            </form>
                        </div>
                    </div>
                </div>
